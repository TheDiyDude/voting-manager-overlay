...
public class CPHInline
{
    ...
    private void ProcessManagerCommand(string input)
    {
        ...
        if (input.Equals("reset", StringComparison.OrdinalIgnoreCase) || input.Equals("restart", StringComparison.OrdinalIgnoreCase))
        {
            ...
            {
                CPH.SendMessage(" No previous poll available to reset.");
            }
            return;
        }
        ...
        if (input.StartsWith("+") || input.StartsWith("-"))
        {
            if (status == "active" || status == "paused")
            {
                ...
                SaveData();
                CPH.SendMessage($" Poll interval adjusted. Remaining: {remainingSec / 60}m {remainingSec % 60}s");
            }
            return;
        }
        ...
        if (timeMatch.Success)
        {
            ...
            if (matches.Count >= 3 && matches.Count <= 11)
            {
                ...
                status = "prepared";
                SaveData();
                CPH.SendMessage($" Poll prepared: \"{title}\" ({options.Count} Optionen). Start with '!poll start'");
            }
            else
            {
                CPH.SendMessage(" Syntax-Error: !poll 10m \"Titel\" \"Option 1\" \"Option 2\" (max 10 Optionen)");
            }
        }
    }
    ...
    private void EndPoll()
    {
        ...
        string chatAnnouncement = $" Poll closed! Results for \"{title}\": Winner is '{winner}' with {displayVotes} Votes (Total: {total} Votes).";
